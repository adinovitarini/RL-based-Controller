function [KG,net] = KalmanNet(delta_x,target,u,A,B,C,N,nh,bs,dl)
%%  Normalize state trajectories
% delta_x = [x_next_nw;y];
% delta_x = x_next_nw;
%%  Propagate 
x_hat_kf = A*delta_x(1:end-size(C,1),:)+B*u;
y_hat_kf = C*x_hat_kf;
%%
X = delta_x;
Y = target(1+size(A,1):end,:);
tic
...
%     fullyConnectedLayer(numFeatures)
    ...
    dropoutLayer(dl,"Name","dropout")
    fullyConnectedLayer(numResponses)
    regressionLayer];
options = trainingOptions('adam', ...
    'MaxEpochs',100, 'MiniBatchSize',bs,...
    'GradientThreshold',.1, ...
